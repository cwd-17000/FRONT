...
function getActiveOrgId(): string | null {
  ...
  // Pass orgId via a data attribute or use a context — simplest: read from a meta tag set by layout
  return null;
}
...
export default function NewGoalPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // First fetch activeOrgId from /api/auth/me since cookie is httpOnly
      const meRes = await fetch("/api/auth/me", { credentials: "include" });
      if (!meRes.ok) { router.push("/login"); return; }
      const me = await meRes.json();

      if (!me.activeOrgId) { router.push("/onboarding"); return; }

      const res = await fetch(`/api/organizations/${me.activeOrgId}/goals`, {
        method: "POST",
